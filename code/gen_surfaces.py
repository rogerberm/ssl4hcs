...
def get_plot_data(csv_data, column_list, reference_point=None):
    if reference_point:
    ...
    result = np.vstack([np.hstack([key.view('float'), np.median([item['f2'] for item in group])]) for key, group in
...
def main(cmdline):
    ...
    a_lab_unlab = get_plot_data(csv_data, [A_LABELED, A_UNLABELED, RECALL], reference_point)
    plot_surface(fig, 1, a_lab_unlab, label_x, label_y, label_z)

    a_lab_s_inf = get_plot_data(csv_data, [A_LABELED, A_S_INFECTED, RECALL], reference_point)
    label_y = columns[A_S_INFECTED]
    plot_surface(fig, 2, a_lab_s_inf, label_x, label_y, label_z)

    a_lab_s_uninf = get_plot_data(csv_data, [A_LABELED, A_S_UNINFECTED, RECALL], reference_point)
    label_y = columns[A_S_UNINFECTED]
    plot_surface(fig, 3, a_lab_s_uninf, label_x, label_y, label_z)

    label_x = columns[A_UNLABELED]
    a_unlab_s_inf = get_plot_data(csv_data, [A_UNLABELED, A_S_INFECTED, RECALL], reference_point)
    label_y = columns[A_S_INFECTED]
    plot_surface(fig, 4, a_unlab_s_inf, label_x, label_y, label_z)

    label_x = columns[A_UNLABELED]
    a_unlab_s_uninf = get_plot_data(csv_data, [A_UNLABELED, A_S_UNINFECTED, RECALL], reference_point)
    label_y = columns[A_S_UNINFECTED]
    plot_surface(fig, 5, a_unlab_s_uninf, label_x, label_y, label_z)

    label_x = columns[A_S_INFECTED]
    a_s_inf_s_uninf = get_plot_data(csv_data, [A_S_INFECTED, A_S_UNINFECTED, RECALL], reference_point)
    label_y = columns[A_S_UNINFECTED]
